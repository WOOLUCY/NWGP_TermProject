...
LRESULT CALLBACK WndProc(HWND hWnd, UINT iMsg, WPARAM wParam, LPARAM lParam)
{
	...
	static CImage Start, Dialog[6], Guide, Block;
	...
	case WM_PAINT:
		...
		for ( int i = 0; i < 1; i++ ){
			Block.Draw(mem1dc, Block_local[i].x - CharX, Block_local[i].y - CharY, Block_local[i].width, 30, 0, 0, w_block, h_block);	// 벽돌
		}
		...
		for (int i = 0; i < 1; i++) {			// 벽돌 만지는 부분. 벽돌 위 좌표값이 좀 이상함
			if (x + CharX >= Block_local[i].x && x + CharX <= Block_local[i].x + Block_local[i].width && Block_local[i].y + 30 >= y + h_stand[count] + CharY) {
					y = Block_local[i].y - h_stand[count] - CharY + 30;
					jump = 0;
					jumpcount = 0;
			}
